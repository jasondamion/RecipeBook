...
var userFunctions = {
  ...
  async SignUp(firstName, username, password) {
    ...
    var signUpStatus = await external.db.signup(
      firstName,
      username,
      hashedPassword
    );
    if (signUpStatus.Result == "Success") {
      ...
      var token = external.auth.genJWTCode(
        signUpStatus.UserId,
        username,
        password
      );
  ...
  async AddRecipe(token, recipeName, recipeId, recipeSummary, recipeComments) {
    var isValid = await external.auth.isValidUser(token);
    if (isValid) {
      return external.db.addRecipe(
        external.auth.getIdFromToken(token),
        recipeId,
        recipeName,
        recipeSummary,
        recipeComments
  ...
  async DeleteRecipe(token, recipeId) {
    var isValid = await external.auth.isValidUser(token);
    if (isValid) {
      return external.db.deleteRecipe(
        external.auth.getIdFromToken(token),
        recipeId
      );
    } else {
  ...
  async EditRecipe(token, recipeId, comments) {
    var isValid = await external.auth.isValidUser(token);
    if (isValid) {
      return external.db.editRecipeComment(
        external.auth.getIdFromToken(token),
        recipeId,
        comments
      );
  ...
  async GetCustomRecipe(token, recipeId) {
    var isValid = await external.auth.isValidUser(token);
    if (isValid) {
      return external.db.getUserCustomRecipe(
        external.auth.getIdFromToken(token),
        recipeId
      );
    } else {
  ...
  async AddCustomRecipe(
    ...
    var isValid = await external.auth.isValidUser(token);
    if (isValid) {
      return external.db.addCustomRecipe(
        external.auth.getIdFromToken(token),
        recipeName,
        recipeIngredients,
        recipeInstructions,
  ...
  async EditCustomRecipe(
    ...
    var isValid = await external.auth.isValidUser(token);
    if (isValid) {
      return external.db.editCustomRecipe(
        external.auth.getIdFromToken(token),
        recipeId,
        recipeName,
        recipeIngredients,
  ...
  async DeleteCustomRecipe(token, recipeId) {
    var isValid = await external.auth.isValidUser(token);
    if (isValid) {
      return external.db.deleteCustomRecipe(
        external.auth.getIdFromToken(token),
        recipeId
      );
    } else {
  ...
  async ForgetPassword(firstName, username, suggestedPassword) {
    var emailStatus = await external.email.ForgetPassword(
      firstName,
      username,
      suggestedPassword
    );
